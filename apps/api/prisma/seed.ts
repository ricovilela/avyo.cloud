...
import {
  colorClasses as realColorClasses,
  officialColors as realOfficialColors,
  type SeedColorClass,
  type SeedOfficialColor,
} from "./seed-data/color-catalog";

/** The two valid `age_group` values (Req 6.5). */
const AGE_GROUPS = ["young", "adult"] as const;
...
/** The dataset consumed by {@link seedColorCatalog}. */
export interface SeedDataset {
  colorClasses: SeedColorClass[];
  officialColors: SeedOfficialColor[];
}

/** Summary of what a seed run converged the catalogs to. */
export interface SeedResult {
  colorClassCount: number;
  officialColorCount: number;
}

/**
 * The minimal Prisma surface the seed needs. Accepts a full `PrismaClient` (or
 * `PrismaService`) so callers — including tests — can pass any client.
 */
type SeedPrismaClient = Pick<PrismaClient, "$transaction">;
 ...
 */
export async function seedColorCatalog(
  ...
  for (const color of officialColors) {
    if (!isValidAgeGroup(color.ageGroup)) {
      throw new SeedError(
        `Invalid age_group "${String(color.ageGroup)}" for official color ` +
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const color of officialColors) {
      ...
      if (classId === undefined) {
        throw new SeedError(
          `Unresolved classCode "${color.classCode}" for official color ` +
...
async function main(): Promise<void> {
  ...
  try {
    const result = await seedColorCatalog(prisma, {
      colorClasses: realColorClasses,
      officialColors: realOfficialColors,
    });
    console.log(
      `Seed complete — ${result.colorClassCount} color class(es), ` +
